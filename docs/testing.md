...
| `tests/golden/` | Golden contract files | The captured stdout and exit status of every command over every fixture, in `--json` and human mode, plus two trusted `verify` runs over the signed XMLDSig vector, one `create` case that builds a dossier from `tests/fixtures/create/`, and two `sign` refusals. Compared by `scripts/golden.py`; see [Golden output contract](#golden-output-contract). |
