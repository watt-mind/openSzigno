...
| `crates/openszigno-verify/tests/common/` | Test helper | A synthetic PKI built with `rcgen` and an XMLDSig signer built on the crate's own canonicalization backend plus `rsa`/`p256`. Signing a dossier is a permanent non-goal of the shipped tool, so this helper lives in `tests/` and must never move into a shipped crate. The RSA keys in `common/keys.rs` are generated, committed test material and must never be used for anything. |
