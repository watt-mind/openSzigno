...
`crates/openszigno-cli/tests/online.rs` binds loopback sockets and includes one
test that waits out the 20-second fetch timeout, so the CLI suite takes about
half a minute longer than the rest. It never leaves the loopback interface.
