...
| Distribution channels | Releases ship tarballs, a zip, and checksums. Next steps: `cargo binstall` metadata, a Homebrew tap, a winget manifest, `.deb`/`.rpm` packages, a GHCR image, and build provenance attestations, most of which `cargo-dist` can generate. |
