...
| LAB-291 | A CSC (Cloud Signature Consortium) remote signing backend, so the key never reaches this process at all. `sign --csc` runs the non-interactive half of CSC API v2: discovery, credential resolution, `credentials/authorize` with a PIN or one-time password, and `signatures/signHash` over the digest alone. An `oauth2`-mode credential was refused with `csc_authorization_required` until LAB-296 added the interactive login it needs. | Done |
...
| LAB-296 | `openszigno csc login`: the OAuth 2.0 authorization-code rounds `--csc` could not run. A loopback redirect listener on `127.0.0.1`, PKCE `S256`, the `scope=service` round and its token store with refresh on expiry, and the `scope=credential` round `sign` runs itself, as query parameters or RFC 9396 `authorization_details` where `supportsRar` is true. It is what makes an `oauth2`-mode credential usable. | Done |
...
- Whether `application/nldossier2` is the right declared type for a general
  ...
  writes it; LAB-292 is where that gets checked against the reference tool.
- What a signature this project writes must cover for the reference tool to
  ...
  agrees is LAB-292, and is the one thing that cannot be settled from inside
  this repository.
...
- Which key transport the Microsec reference tool writes. Its documented
  ...
  reference tool actually wants is a LAB-292 question.
