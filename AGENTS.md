...
- **Key material never leaves the process.** `extract --decrypt-key` and
  `sign --key` take the key, its certificate, and its passphrase from files (or
  the passphrase from `OPENSZIGNO_DECRYPT_PASSPHRASE`) and never from `argv`.
  Key bytes, the passphrase, and anything derived from them must never appear
  in a log,
