...
pub(crate) fn parse(bytes: &[u8], limits: &Limits) -> Result<Dossier, Error> {
    ...
    if contains_ascii_case_insensitive(xml.as_bytes(), b"<!DOCTYPE")
        || contains_ascii_case_insensitive(xml.as_bytes(), b"<!ENTITY")
    {
        return Err(Error::new(
            ErrorCode::UnsafeXml,
    ...
    let tree = XmlDocument::parse(&xml).map_err(|error| {
        Error::new(
            ErrorCode::InvalidXml,
            format!("XML parsing failed: {error}"),
        )
    })?;
    ...
    enforce_depth(root, limits.max_xml_depth)?;
...
fn declared_encoding(bytes: &[u8]) -> Option<String> {
    ...
    let encoding_pos = lower.find("encoding")? + "encoding".len();
    let tail = ascii.get(encoding_pos..)?;
...
fn enforce_depth(root: Node<'_, '_>, max_depth: usize) -> Result<(), Error> {
    let mut depths = HashMap::new();
    depths.insert(root.id(), 1usize);
    for node in root.descendants().filter(Node::is_element).skip(1) {
        let parent_depth = node
            .parent_element()
            .and_then(|parent| depths.get(&parent.id()).copied())
            .unwrap_or(0);
        let depth = parent_depth + 1;
        if depth > max_depth {
            return Err(Error::new(
                ErrorCode::UnsafeXml,
                format!("XML nesting exceeds {max_depth} elements"),
            ));
        }
