...
| `--trust-list-signer CERT` | `verify` | Certificate, PEM or DER, that must have signed every `--trust-list`. Obtain it out of band — for the EU list of trusted lists, from the Official Journal. Without it the lists are read but reported `trust_list_unverified`, which caps the verdict at `indeterminate`. |
